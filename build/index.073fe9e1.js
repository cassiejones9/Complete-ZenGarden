...
/*******************
		buttons
  ********************/ const buttonsWrapper = document.querySelector('#buttons .cd-box'), buttonsHtml = buttonsWrapper.innerHTML;
...
buttonsHtmlText.map(function(value) {
    if (value.indexOf('button') >= 0) {
        const splitText = value.split('class="'), block1 = splitText[0] + 'class="';
...
/*******************
		typography
	********************/ const heading = document.querySelector('#typography h1'), headingDescriptionText = heading.children[0], body = heading.nextElementSibling, bodyDescriptionText = body.children[0];
...
function setTypography(element, textElement) {
    ...
    let fontSize = Math.round(cs.fontSize.replace('px', '')) + 'px', fontFamily = cs.fontFamily.split(',')[0].replace(/\'/g, '').replace(/\"/g, ''), fontWeight = cs.fontWeight;
...
document.querySelectorAll('.cd-main-nav a[href^="#"]').forEach(function(link) {
    link.addEventListener('click', function(event) {
        ...
        const target = document.querySelector(this.hash), topMargin = document.defaultView.getComputedStyle(target, null).marginTop.replace('px', ''), headerHeight = header.scrollHeight;
...
function updateNavigation() {
    contentSections.forEach(function(target) {
        ...
        const actualHeight = actual.height, topMargin = cs.marginTop.replace('px', ''), actualAnchor = document.querySelector('.cd-main-nav a[href="#' + actual.id + '"]');
        if (parseInt(actual.offsetTop - document.querySelector('.cd-main-nav').height - topMargin) <= window.scrollY && parseInt(actual.offsetTop + actualHeight - topMargin) > window.scrollY + 1) actualAnchor.classList.add('selected');
        else actualAnchor.classList.remove('selected');
    });
}

//# sourceMappingURL=index.073fe9e1.js.map
